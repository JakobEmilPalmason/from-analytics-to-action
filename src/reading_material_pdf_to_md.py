"""Convert all PDF files in the reading-material directories to Markdown."""
...
def convert_all_pdfs(base_dir: Path, overwrite: bool = False):
    """Find all PDFs under base_dir and convert each to a .md file alongside it."""
    pdf_files = sorted(
        p for p in base_dir.rglob("*.pdf")
        if "reading-material" in p.parts
    )
